...
{
    // const_stmt: 'const' NAME '=' test
    identifier name;
    expr_ty expression;

    // Find the name of the const variable
    REQ(n, const_stmt);
    name = CHILD(n, 1);
    if (!name)
        return NULL;

    // Find the value of the const variable
    expression = ast_for_expr(c, CHILD(n, 3));
    if (!expression)
        return NULL;

    return Const(name, expression, LINENO(n), n->n_col_offset, c->c_arena);
}
